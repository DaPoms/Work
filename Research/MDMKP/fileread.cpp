...
/* 
    ...
    Case 6: q = m
*/
void separateCandidatesByCases(MDMKRawProblem& problem, vector<vector<MDMKCandidate>>& candidatesByCase) 
{
    candidatesByCase.resize(6); // new thing learned! Can be used to easily create uninitialized values inside of the vector (or reshape vector to fit this function's usage)
    ...
    for(int i{0}; i < candidateCount; i++)
    {
        ...
        for(int caseNum{0}; caseNum < 6; caseNum++) //Changes candidate to each case (note that caseNum = 0 is case 1 and caseNum = 5 is case 6)
        {
            ...
            for(int currDemand{0}; currDemand < capacityVarsCount; currDemand++) //stops at case 6 (where same # of demands as capacity constraints)
            {
                ...
                {
                    candidatesByCase[currentCase].push_back(candidate);
...
void RawProblemsToCases(vector<MDMKRawProblem>& problems, vector< vector < vector <MDMKCandidate> > >& cases)
{
    vector<vector<MDMKCandidate>> candidatesByCase;
    candidatesByCase.resize(6);
    for(MDMKRawProblem problem : problems)
    {
        separateCandidatesByCases(problem, candidatesByCase);
        cases.push_back(candidatesByCase);
        candidatesByCase.clear();
    }
}
...
int main()
{
    ...
    vector<vector<MDMKCandidate>> candidatesByCase;
    vector<vector<vector<MDMKCandidate>>> problemSets;
