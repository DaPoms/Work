...
select address, bdrms from property_ID where office_num IN
(
	select office_num from office where office_name = 'StayWell-Colombia City'
);

select property_ID, address, bdrms from property where (bdrms between 2 AND 4) AND office_num IN -- alternate version of the above, can change the bedroom range to look for specifications
(
	select office_num from office where office_name = 'StayWell-Colombia City'
);

select property_ID, property.address, bdrms from office, property -- equivalence to above, but using joining
where 
    office.office_num = property.office_num 
	AND office_name = 'StayWell-Colombia City'
    AND bdrms between 2 AND 4;
