...
/* Project:        #1                                       */
/* Filename:       warmup.ml                                */
/* Purpose:        Impliment basic OCaml functions to       */
...
let rec len x = 
  if(x < 0) then (*Because the assumption isnt given that x is positive only, this accounts for negative numbers (but makes a fairly unnecessary comparison each time after the 1st run)*)
    len( abs x )
  else if(x < 10) then (*we need to do this to account for len 0 should = 1 but also to not discount a single digit that isnt 0*)
